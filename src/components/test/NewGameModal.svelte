<script lang="ts">
	...
	let { onClose, onNewGameOpened }: { onClose: () => void , onNewGameOpened: () => void} = $props();

	let newgamemodalOpened: boolean = $state(false);
	let gameOptions: {name: string, maxPlayers: number, time: {minutes: number, seconds: number}, timeformove: { minutes: number, seconds: number }, id: number, spectate: boolean} 
	= $state({name: 'Новая игра', maxPlayers: 2, time: {minutes: 10, seconds: 0}, timeformove: {minutes: 0, seconds: 30}, id: 0, spectate: false});
...
<div class="shadow">
    ...
    <svg class="vector" width="40" height="37" viewBox="0 0 40 37" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <svg class="line" width="347" height="1" viewBox="0 0 347 1" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<input type="checkbox" class="toggle" bind:checked={gameOptions.spectate}>
				...
				<svg class="line1" width="188" height="1" viewBox="0 0 188 1" fill="none" xmlns="http://www.w3.org/2000/svg">
					<line y1="0.5" x2="188" y2="0.5" stroke="url(#paint0_linear_22_46)"/>
					<defs>
					<linearGradient id="paint0_linear_22_46" x1="0" y1="1.5" x2="188" y2="1.5" gradientUnits="userSpaceOnUse">
					<stop stop-color="#EB7445" stop-opacity="0"/>
					<stop offset="0.545" stop-color="#EB7445"/>
					<stop offset="1" stop-color="#EB7445" stop-opacity="0"/>
					</linearGradient>
